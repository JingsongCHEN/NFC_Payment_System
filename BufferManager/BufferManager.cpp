#include "BufferManager.h" 
//Get a new block 
Block* MBuffer::GBlock()
{
	Block* Tmp;
	Tmp = new Block;
	Tmp->record = new char[BlockSize];
	memset(Tmp->record,0,BlockSize);
	Tmp->tablename = "";
	Tmp->offset = 0;
	Tmp->accessed = false;
	Tmp->written = false;	
	return Tmp;
}
//Initialize the buffer
bool MBuffer::Init()
{
	int i;
	//Get 64 blocks
	for(i=0;i<64;i++)
	{
		Buffer[i] = GBlock();
		if(Buffer[i] == NULL || Buffer[i]->record == NULL)
			return false;
	}
	return true;
}
//Get a block from the buffer 
Block* MBuffer::GetBlock(string tablename, int offset, int flag)
{
	Block* Target;
	int i;
	//check if the wanted block is in the buffer
	for(i=0;i<64;i++)
	{
		Target = Buffer[i];
        if(Target->tablename == tablename && Target->offset == offset)
		{
			Target->accessed = true;
			return Target;
		}
	}
	//if the block is from the file
	if(flag == 0)
	{ 	
		if(Sche1(tablename,offset))//schedule the buffer
		{
			for(i=0;i<64;i++)//find the wanted block
			{
				Target = Buffer[i];
				if(Target->tablename == tablename && Target->offset == offset)
				{
					Target->accessed = true;
					return Target;
				}
			}
		}
	} 
	//if the block is not from the file,then create a new block
	else
	{
		if(Sche2(tablename,offset))//schedule the buffer
		{
			for(i=0;i<64;i++)//find the wanted block
			{
				Target = Buffer[i];
				if(Target->tablename == tablename && Target->offset == offset)
				{
					Target->accessed = true;
					return Target;
				}
			}
		}
	} 
	return NULL;
}
//to schedule the buffer when the block is from file
bool MBuffer::Sche1(string tablename, int offset)
{
	int i = 0;
	//to find an idle block in the buffer
	for(i=0;i<64;i++)
	{
		if(Buffer[i]->tablename == "")
		{
			break;			
		}
	}
	//if there is no idle block in the buffer(LRU)
	if(i==64)
	{
		i = 0; 
		while(1)
		{
			if(Buffer[i]->accessed == false)
				break;
			else
				Buffer[i]->accessed = false;
			i++;
			if(i == 64)
				i = 0;	
		}
	}
	//exchange the block between the buffer and the file 
	Exchange(tablename, offset, Buffer[i]);
	if(Buffer[i] != NULL)
		return true;
	else 
		return false;
}
//to schedule the buffer when the block is not from file
bool MBuffer::Sche2(string tablename, int offset)
{
	int i = 0;
	//to find an idle block in the buffer
	for(i=0;i<64;i++)
	{
		if(Buffer[i]->tablename == "")
		{
			break;		
		}
	}
	//if there is no idle block in the buffer(LRU)
	if(i==64)
	{
		i = 0;
		while(1)
		{
			if(Buffer[i]->accessed == false)
				break;
			else
				Buffer[i]->accessed = false;
			i++;
			if(i == 64)
				i = 0;	
		}
	}
	//save the replaced block in the file
	Exchange("",-1,Buffer[i]);
	//create a new block
	Buffer[i]->tablename = tablename;
	Buffer[i]->accessed = true;
	Buffer[i]->written = true;
	Buffer[i]->offset = offset;
	memset(Buffer[i]->record,0,BlockSize);
	//save the new block in the file
	Exchange("",-1,Buffer[i]);
	if(Buffer[i] != NULL)
		return true;
	else 
		return false;
}
//to delete the blocks of the dropped table
bool MBuffer::Drop(string tablename)
{
	Block* Target;
	int i;
	//find the wanted blocks
	for(i=0;i<64;i++)
	{
		Target = Buffer[i];
		if(Target->tablename == tablename)
		{
			//create a new block
			Buffer[i] = GBlock();
			if(Buffer[i] == NULL || Buffer[i]->record == NULL)
				return false;
			//delete the old block
			delete Target->record;
			delete Target;
		}
	}
	return true;
}
//exchange the block between the buffer and the file 
bool  MBuffer::Exchange(string tablename, int offset, Block* Replaced)
{	
	FILE* fp;
	int i;
	char Filename[64];
	memset(Filename,0,sizeof(Filename));
	int length = 0;
	length = Replaced->tablename.size();
	if(length!=0)
	{
		//get the filename of replaced block 
		for(i=0;i<length;i++)
		{
			Filename[i] = Replaced->tablename.at(i);
		}
		//write back to the file
		if((fp = fopen(Filename,"rb+"))==NULL)
		{
			return false;
		} 
		fseek(fp,BlockSize*Replaced->offset,SEEK_SET);
		fwrite(Replaced->record,BlockSize,1,fp);
		fclose(fp);
	}
	memset(Filename,0,sizeof(Filename));
	//if need to read a block from the file
	if(offset!=-1)
	{
		length = tablename.size();
		//get the filename
		for(i=0;i<length;i++)
		{
			Filename[i] = tablename.at(i);
		}
		//read the block from the file
		if((fp = fopen(Filename,"rb"))==NULL)
		{
			cout<<"Reading data error!"<<endl;
			return false;
		} 
		fseek(fp,BlockSize*offset,SEEK_SET);
		fread(Replaced->record,BlockSize,1,fp);		
		fclose(fp);
		Replaced->offset = offset;
		Replaced->accessed = true;
		Replaced->tablename = tablename;
		return true;
	}
	else
		return true;
}
 
MBuffer::~MBuffer()
{
	Block* Target;
	int i;
	for(i=0;i<64;i++)
	{
		Target=Buffer[i];
		if(Target!=NULL)
		{
			//write back to the file
			if(Target->tablename != "")
				Exchange("",-1,Target);
			//delete the pointer
			delete Target->record;
			delete Target;
		}
	}
}
//clear the buffer
void MBuffer::clearBuffer()
{
    Block* Target;
    int i;
    for(i=0;i<64;i++)
    {
        Target=Buffer[i];
        //write back to the file
        if(Target!=NULL)
        {
            if(Target->tablename != "")
                Exchange("",-1,Target);
        }
        //real clear buffer
        memset(Buffer[i]->record,0,BlockSize);
        Buffer[i]->tablename = "";
        Buffer[i]->offset = 0;
        Buffer[i]->accessed = false;
        Buffer[i]->written = false;
    }
}
//calculate the number of block of a file
int Block_num(string file_name)
{
	int num = 0;
	FILE *fp;
	fp = fopen(file_name.c_str(),"r");
	fseek(fp, 0, 2);  
    num = ftell(fp)/BlockSize;  
    fclose(fp);
    return num;
}